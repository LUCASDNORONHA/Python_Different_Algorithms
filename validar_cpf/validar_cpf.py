...
entrada = input('Digite seu CPF sem pontos ou traços: ')

#Se o usuário digitar ponto ou traço, os tais caracteres serão removidos para que fique somente os caracteres númericos.
CPF_usuario = re.sub( r'[^0-9]', '', entrada )

# Uma variável para armazenar valor verdadeiro ou falso caso o usuário tenha digitado caracteres repetidos sequêncialmente.
entrada_caractere_repetidos = entrada == entrada[0] * len(entrada)

# Se o usuário digitar caracteres repetidos, então o seguinte bloco será executado.
if entrada_caractere_repetidos:
    print('Você digitou caracteres sequênciais.')
    sys.exit()
